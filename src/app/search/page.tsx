    ...
    <main className="min-h-screen bg-white pt-20 sm:pt-24 pb-12 sm:pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="mb-6 sm:mb-8">
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 mb-1">Search Results</h1>
          {query ? (
            <p className="text-gray-500 text-sm">{results.length} result{results.length !== 1 ? 's' : ''} for &quot;{query}&quot;</p>
          ) : (
            <p className="text-gray-500 text-sm">Enter a search term to find products</p>
          )}
        </motion.div>
